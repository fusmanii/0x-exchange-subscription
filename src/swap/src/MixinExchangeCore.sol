    ...
    MixinSignatureValidator
{
        ...
        refundFinalBalanceNoReentry
    {
        ...
        address orderSenderAddress = makerAddress == msg.sender
            ? address(0)
            : msg.sender;
        ...
        if (newOrderEpoch <= oldOrderEpoch) {
            LibRichErrors.rrevert(
                LibExchangeRichErrors.OrderEpochError(
                    makerAddress,
                    orderSenderAddress,
                    oldOrderEpoch
                )
        ...
        emit CancelUpTo(makerAddress, orderSenderAddress, newOrderEpoch);
    }
    ...
    /// @dev Cancels the original offer and then accepts the counter offer.
    /// @param originalOrder Order struct of the original.
    /// @param counterOrder Order struct containing counter order specifications.
    /// @param takerAssetFillAmount Desired amount of takerAsset to sell.
    /// @param signature Proof that order has been created by maker.
    /// @return fillResults Amounts filled and fees paid by maker and taker.
    function fillCounterOrder(
        ...
        returns (LibFillResults.FillResults memory fillResults)
    {
        ...
        fillResults = _fillOrder(counterOrder, takerAssetFillAmount, signature);
        return fillResults;
    }
    ...
    function _fillOrder(
        ...
        _onlyActiveSubscription(order.makerAddress, msg.sender);
