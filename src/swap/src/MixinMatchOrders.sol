...
contract MixinMatchOrders is MixinExchangeCore, IMatchOrders {
    ...
    function batchMatchOrders(
        ...
        returns (
            LibFillResults.BatchMatchedFillResults
                memory batchMatchedFillResults
        )
    {
        return
            _batchMatchOrders(
    ...
    function _assertValidMatch(
        ...
        if (
            ...
            LibRichErrors.rrevert(
                LibExchangeRichErrors.NegativeSpreadError(
                    leftOrderHash,
                    rightOrderHash
                )
            );
        ...
        )
    {
        ...
        if (leftOrders.length == 0) {
            LibRichErrors.rrevert(
                LibExchangeRichErrors.BatchMatchOrdersError(
        ...
        batchMatchedFillResults.left = new LibFillResults.FillResults[](
            leftOrders.length
        );
        batchMatchedFillResults.right = new LibFillResults.FillResults[](
            rightOrders.length
        );
        ...
        ) = _getOrderHashAndFilledAmount(leftOrder);
        (
            ,
            uint256 rightOrderTakerAssetFilledAmount
        ) = _getOrderHashAndFilledAmount(rightOrder);
        ...
        for (;;) {
            ...
            LibFillResults.MatchedFillResults
                memory matchResults = _matchOrders(
                    leftOrder,
                    rightOrder,
            ...
            leftOrderTakerAssetFilledAmount = leftOrderTakerAssetFilledAmount
                .safeAdd(matchResults.left.takerAssetFilledAmount);
            rightOrderTakerAssetFilledAmount = rightOrderTakerAssetFilledAmount
                .safeAdd(matchResults.right.takerAssetFilledAmount);
            ...
            batchMatchedFillResults
                .profitInLeftMakerAsset = batchMatchedFillResults
                .profitInLeftMakerAsset
                .safeAdd(matchResults.profitInLeftMakerAsset);
            batchMatchedFillResults
                .profitInRightMakerAsset = batchMatchedFillResults
                .profitInRightMakerAsset
                .safeAdd(matchResults.profitInRightMakerAsset);
            ...
            if (
                rightOrderTakerAssetFilledAmount >= rightOrder.takerAssetAmount
            ) {
