    ...
    MixinTransactions
{
    ...
    function preSign(
        bytes32 hash
    ) external payable refundFinalBalanceNoReentry {
    ...
    function isValidOrderSignature(
        ...
        isValid = _isValidOrderWithHashSignature(order, orderHash, signature);
        return isValid;
    }
    ...
    function _validateBytesWithValidator(
        ...
        if (!allowedValidators[signerAddress][validatorAddress]) {
            LibRichErrors.rrevert(
                LibExchangeRichErrors.SignatureValidatorNotApprovedError(
                    signerAddress,
                    validatorAddress
                )
            );
    ...
    function _staticCallEIP1271WalletWithReducedSignatureLength(
        ...
        signature.writeLength(signatureLength);
        // Static call the verification function
        (bool didSucceed, bytes memory returnData) = verifyingContractAddress
            .staticcall(callData);
        // Return the validity of the signature if the call was successful
        if (didSucceed && returnData.length == 32) {
        ...
        // Revert if the call was unsuccessful
        LibRichErrors.rrevert(
            LibExchangeRichErrors.EIP1271SignatureError(
