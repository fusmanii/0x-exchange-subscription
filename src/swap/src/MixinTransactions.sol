    ...
    ITransactions
{
    ...
    ) public payable disableRefundUntilEnd returns (bytes memory) {
    ...
    ) public payable disableRefundUntilEnd returns (bytes[] memory returnData) {
    ...
    function _executeTransaction(
        ...
        bytes32 transactionHash = transaction.getTypedDataHash(
        ...
        if (!didSucceed) {
            LibRichErrors.rrevert(
                LibExchangeRichErrors.TransactionExecutionError(
                    transactionHash,
                    returnData
                )
            );
    ...
    function _assertExecutableTransaction(
        ...
        if (tx.gasprice != requiredGasPrice) {
            LibRichErrors.rrevert(
                LibExchangeRichErrors.TransactionGasPriceError(
                    transactionHash,
                    tx.gasprice,
                    requiredGasPrice
                )
    ...
    function _getCurrentContextAddress() internal view returns (address) {
        address currentContextAddress_ = currentContextAddress;
        address contextAddress = currentContextAddress_ == address(0)
            ? msg.sender
            : currentContextAddress_;
