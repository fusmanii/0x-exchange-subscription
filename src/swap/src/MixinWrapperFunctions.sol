...
contract MixinWrapperFunctions is IWrapperFunctions, MixinExchangeCore {
    ...
    {
        fillResults = _fillOrKillOrder(order, takerAssetFillAmount, signature);
        return fillResults;
    }

    /// @dev Cancels the original order, then fills the input order. Reverts if exact `takerAssetFillAmount` not filled.
    /// @param originalOrder Order struct of the original.
    /// @param counterOrder Order struct containing order specifications.
    /// @param takerAssetFillAmount Desired amount of takerAsset to sell.
    /// @param signature Proof that order has been created by maker.
    /// @return fillResults Amounts filled and fees paid.
    function fillOrKillCounterOrder(
        LibOrder.Order memory originalOrder,
        LibOrder.Order memory counterOrder,
        uint256 takerAssetFillAmount,
        bytes memory signature
    )
        public
        ...
        returns (LibFillResults.FillResults memory fillResults)
    {
        ...
        for (uint256 i = 0; i != ordersLength; i++) {
            ...
            uint256 remainingTakerAssetFillAmount = takerAssetFillAmount
                .safeSub(fillResults.takerAssetFilledAmount);

            // Attempt to sell the remaining amount of takerAsset
            LibFillResults.FillResults
                memory singleFillResults = _fillOrderNoThrow(
                    orders[i],
            ...
            fillResults = LibFillResults.addFillResults(
                fillResults,
                singleFillResults
            );
        ...
        returns (LibFillResults.FillResults memory fillResults)
    {
        ...
        for (uint256 i = 0; i != ordersLength; i++) {
            ...
            uint256 remainingMakerAssetFillAmount = makerAssetFillAmount
                .safeSub(fillResults.makerAssetFilledAmount);

            // Convert the remaining amount of makerAsset to buy into remaining amount
            // of takerAsset to sell, assuming entire amount can be sold in the current order
            uint256 remainingTakerAssetFillAmount = LibMath
    ...
    function marketSellOrdersFillOrKill(
        ...
        uint256 takerAssetFillAmount,
        bytes[] memory signatures
    ) public payable returns (LibFillResults.FillResults memory fillResults) {
        fillResults = marketSellOrdersNoThrow(
            orders,
            takerAssetFillAmount,
            signatures
        );
        if (fillResults.takerAssetFilledAmount < takerAssetFillAmount) {
            LibRichErrors.rrevert(
                LibExchangeRichErrors.IncompleteFillError(
    ...
    function marketBuyOrdersFillOrKill(
        ...
        fillResults = marketBuyOrdersNoThrow(
            orders,
    ...
    function batchCancelOrders(
        LibOrder.Order[] memory orders
    ) public payable refundFinalBalanceNoReentry {
        uint256 ordersLength = orders.length;
        for (uint256 i = 0; i != ordersLength; i++) {
    ...
    /// @param order Order struct containing order specifications.
    /// @param takerAssetFillAmount Desired amount of takerAsset to sell.
    /// @param fillResults signature Proof that order has been created by maker.
    ...
    function _fillOrderNoThrow(
        ...
        (bool didSucceed, bytes memory returnData) = address(this).delegatecall(
            fillOrderCalldata
        );
        if (didSucceed) {
